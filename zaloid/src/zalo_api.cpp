...
// Zalo API nhan tham so "data" la mot JSON object nam trong query string,
// can ma hoa URL truoc khi gan vao URL.
...
nlohmann::json get_json(HttpClient& http, const std::string& url,
...
std::vector<User> ZaloApi::list_users(int offset, int count) {
    ...
    std::string url = std::string(kBase) + "/v3.0/oa/user/getlist?data=" +
                      url_encode(data.dump());
...
std::vector<Message> ZaloApi::list_messages(const std::string& user_id,
                                            int offset, int count) {
    if (count > 10) count = 10; // API gioi han 10 tin nhan moi request
    nlohmann::json data = {{"user_id", std::stoll(user_id)},
                           {"offset", offset}, {"count", count}};
    std::string url = std::string(kBase) + "/v2.0/oa/conversation?data=" +
                      url_encode(data.dump());
